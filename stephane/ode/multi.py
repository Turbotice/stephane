...
def compute_multi(savefolder,L):
    j = L[0]
    theta = L[1]
    Dt = L[2]
    sigma = L[3]
...
def run(savefolder):
    ...
    tlist = np.linspace(0,theta,3)
    nt = len(tlist)
    dtlist = Dt*np.ones(nt)
    slist = sigma*np.ones(nt)

    Ncpu = os.cpu_count()
    with Pool(processes=2) as pool:
        ite = zip(np.arange(nt),tlist,dtlist,slist)
	        #ite = [(0,25), (26, 50), (51, 75), (76, 100)]
        func = partial(compute_multi,savefolder)
        f = pool.map(func, ite)
